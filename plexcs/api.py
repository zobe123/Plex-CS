...
from plexcs import versioncheck, logger, plextv, pmsconnect, datafactory, graphs, users
import os
import plexcs
import json
import traceback
...
class Api(object):
    ...
    def checkParams(self, *args, **kwargs):
        ...
        if self.apikey == plexcs.CONFIG.API_KEY and plexcs.CONFIG.API_ENABLED and self.cmd in cmd_list:
            self.authenticated = True
            self.msg = None
        elif self.cmd == 'getApikey' and plexcs.CONFIG.API_ENABLED:
            self.authenticated = True
    ...
    def _getApikey(self, username='', password=''):
        ...
        if plexcs.CONFIG.HTTP_USERNAME and plexcs.CONFIG.HTTP_PASSWORD:
            if username == plexcs.HTTP_USERNAME and password == plexcs.CONFIG.HTTP_PASSWORD:
                if plexcs.CONFIG.API_KEY:
                    self.data = plexcs.CONFIG.API_KEY
                else:
                    self.data = apikey
                    plexcs.CONFIG.API_KEY = apikey
                    plexcs.CONFIG.write()
            else:
    ...
    def _getLogs(self, sort='', search='', order='desc', regex='', **kwargs):
        ...
        """
        logfile = os.path.join(plexcs.CONFIG.LOG_DIR, 'plexcs.log')
        templog = []
    ...
    def _getVersion(self, **kwargs):
        self.data = {
            'git_path': plexcs.CONFIG.GIT_PATH,
            'install_type': plexcs.INSTALL_TYPE,
            'current_version': plexcs.CURRENT_VERSION,
            'latest_version': plexcs.LATEST_VERSION,
            'commits_behind': plexcs.COMMITS_BEHIND,
        }
        self.result_type = 'success'
    ...
    def _shutdown(self, **kwargs):
        ...
        self.msg = 'Shutting down plexcs'
        self.result_type = 'success'

    def _restart(self, **kwargs):
        plexcs.SIGNAL = 'restart'
        self.msg = 'Restarting plexcs'
        self.result_type = 'success'

    def _update(self, **kwargs):
        plexcs.SIGNAL = 'update'
        self.msg = 'Updating plexcs'
        self.result_type = 'success'
    ...
    def _getSettings(self):
        ...
        config = {
            ...
            "launch_browser": bool(plexcs.CONFIG.LAUNCH_BROWSER),
            "enable_https": bool(plexcs.CONFIG.ENABLE_HTTPS),
            "https_cert": plexcs.CONFIG.HTTPS_CERT,
            "https_key": plexcs.CONFIG.HTTPS_KEY,
            "api_enabled": plexcs.CONFIG.API_ENABLED,
            "api_key": plexcs.CONFIG.API_KEY,
            "update_db_interval": plexcs.CONFIG.UPDATE_DB_INTERVAL,
            "freeze_db": bool(plexcs.CONFIG.FREEZE_DB),
            "log_dir": plexcs.CONFIG.LOG_DIR,
            "cache_dir": plexcs.CONFIG.CACHE_DIR,
            "check_github": bool(plexcs.CONFIG.CHECK_GITHUB),
            ...
            "pms_ssl": bool(plexcs.CONFIG.PMS_SSL),
            "pms_use_bif": bool(plexcs.CONFIG.PMS_USE_BIF),
            "pms_uuid": plexcs.CONFIG.PMS_UUID,
            "date_format": plexcs.CONFIG.DATE_FORMAT,
            "time_format": plexcs.CONFIG.TIME_FORMAT,
            "grouping_global_history": bool(plexcs.CONFIG.GROUPING_GLOBAL_HISTORY),
            "grouping_user_history": bool(plexcs.CONFIG.GROUPING_USER_HISTORY),
            "grouping_charts": bool(plexcs.CONFIG.GROUPING_CHARTS),
            "movie_notify_enable": bool(plexcs.CONFIG.MOVIE_NOTIFY_ENABLE),
            "tv_notify_enable": bool(plexcs.CONFIG.TV_NOTIFY_ENABLE),
            "music_notify_enable": bool(plexcs.CONFIG.MUSIC_NOTIFY_ENABLE),
            "tv_notify_on_start": bool(plexcs.CONFIG.TV_NOTIFY_ON_START),
            "movie_notify_on_start": bool(plexcs.CONFIG.MOVIE_NOTIFY_ON_START),
            "music_notify_on_start": bool(plexcs.CONFIG.MUSIC_NOTIFY_ON_START),
            "tv_notify_on_stop": bool(plexcs.CONFIG.TV_NOTIFY_ON_STOP),
            "movie_notify_on_stop": bool(plexcs.CONFIG.MOVIE_NOTIFY_ON_STOP),
            "music_notify_on_stop": bool(plexcs.CONFIG.MUSIC_NOTIFY_ON_STOP),
            "tv_notify_on_pause": bool(plexcs.CONFIG.TV_NOTIFY_ON_PAUSE),
            "movie_notify_on_pause": bool(plexcs.CONFIG.MOVIE_NOTIFY_ON_PAUSE),
            "music_notify_on_pause": bool(plexcs.CONFIG.MUSIC_NOTIFY_ON_PAUSE),
            "monitoring_interval": plexcs.CONFIG.MONITORING_INTERVAL,
            "refresh_users_interval": plexcs.CONFIG.REFRESH_USERS_INTERVAL,
            "refresh_users_on_startup": bool(plexcs.CONFIG.REFRESH_USERS_ON_STARTUP),
            "ip_logging_enable": bool(plexcs.CONFIG.IP_LOGGING_ENABLE),
            "movie_logging_enable": bool(plexcs.CONFIG.MOVIE_LOGGING_ENABLE),
            "tv_logging_enable": bool(plexcs.CONFIG.TV_LOGGING_ENABLE),
            "music_logging_enable": bool(plexcs.CONFIG.MUSIC_LOGGING_ENABLE),
            "logging_ignore_interval": plexcs.CONFIG.LOGGING_IGNORE_INTERVAL,
            "pms_is_remote": bool(plexcs.CONFIG.PMS_IS_REMOTE),
            ...
            "home_stats_type": bool(plexcs.CONFIG.HOME_STATS_TYPE),
