...
from plexcs import versioncheck, logger, activity_pinger, plextv, pmsconnect
import plexcs.config
...
def initialize_scheduler():
    ...
    with SCHED_LOCK:
        ...
        schedule_job(database.make_backup, 'Backup Plex:CS database', hours=6, minutes=0, seconds=0, args=(True, True))
...
def shutdown(restart=False, update=False):
    ...
    if restart:
        ...
        if '--nolaunch' not in args:
            args += ['--nolaunch']
        logger.info('Restarting Plex:CS with %s', args)
        os.execv(exe, args)
