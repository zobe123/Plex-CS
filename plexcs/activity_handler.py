...
from plexcs import logger, pmsconnect, activity_processor, threading, notification_handler
...
class ActivityHandler(object):
    ...
    def on_start(self):
        if self.is_valid_session():
            logger.debug(u"Plex:CS ActivityHandler :: Session %s has started." % str(self.get_session_key()))
    ...
    def on_stop(self, force_stop=False):
        if self.is_valid_session():
            logger.debug(u"Plex:CS ActivityHandler :: Session %s has stopped." % str(self.get_session_key()))
    ...
    def on_pause(self):
        if self.is_valid_session():
            logger.debug(u"Plex:CS ActivityHandler :: Session %s has been paused." % str(self.get_session_key()))
    ...
    def on_resume(self):
        if self.is_valid_session():
            logger.debug(u"Plex:CS ActivityHandler :: Session %s has been resumed." % str(self.get_session_key()))
    ...
    def on_buffer(self):
        if self.is_valid_session():
            logger.debug(u"Plex:CS ActivityHandler :: Session %s is buffering." % self.get_session_key())
            ap = activity_processor.ActivityProcessor()
            ...
            logger.debug(u"Plex:CS ActivityHandler :: Session %s buffer count is %s." %
            ...
            if buffer_last_triggered:
                logger.debug(u"Plex:CS ActivityHandler :: Session %s buffer last triggered at %s." %
                             (self.get_session_key(), buffer_last_triggered))
                time_since_last_trigger = int(time.time()) - int(buffer_last_triggered)

            if current_buffer_count >= plexcs.CONFIG.BUFFER_THRESHOLD and time_since_last_trigger == 0 or \
                            time_since_last_trigger >= plexcs.CONFIG.BUFFER_WAIT:
...
class TimelineHandler(object):
    ...
    def on_created(self):
        if self.is_item():
            logger.debug(u"Plex:CS TimelineHandler :: Library item %s has been added to Plex." % str(self.get_rating_key()))
