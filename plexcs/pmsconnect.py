...
from plexcs import logger, helpers, users, http_handler, common
...
def get_server_friendly_name():
    ...
    if not server_name:
        ...
        for server in servers_info:
            if server['machine_identifier'] == plexcs.CONFIG.PMS_IDENTIFIER:
                server_name = server['name']
                break
    
    if server_name and server_name != plexcs.CONFIG.PMS_NAME:
        plexcs.CONFIG.__setattr__('PMS_NAME', server_name)
        plexcs.CONFIG.write()
...
class PmsConnect(object):
    ...
    def __init__(self):
        if plexcs.CONFIG.PMS_URL:
            url_parsed = urlparse(plexcs.CONFIG.PMS_URL)
            hostname = url_parsed.hostname
            port = url_parsed.port
            self.protocol = url_parsed.scheme
        else:
            hostname = plexcs.CONFIG.PMS_IP
            port = plexcs.CONFIG.PMS_PORT
            self.protocol = 'http'

        self.request_handler = http_handler.HTTPHandler(host=hostname,
                                                        port=port,
                                                        token=plexcs.CONFIG.PMS_TOKEN)
    ...
    def get_session_each(self, stream_type='', session=None):
        ...
        if stream_type == 'track':
            ...
            if plexcs.CONFIG.PMS_USE_BIF and indexes == 'sd':
                use_indexes = 1
            else:
                use_indexes = 0
