...
from plexcs import logger, pmsconnect, notification_handler, log_reader, database
...
class ActivityProcessor(object):
        ...
        if session:
            ...
            if result == 'insert':
                ...
                if not session['ip_address']:
                    if plexcs.CONFIG.IP_LOGGING_ENABLE and plexcs.CONFIG.PMS_LOGS_FOLDER:
    ...
    def write_session_history(self, session=None, import_metadata=None, is_import=False, import_ignore_interval=0):
        ...
        if session:
            ...
            if plexcs.CONFIG.MOVIE_LOGGING_ENABLE and str(session['rating_key']).isdigit() and \
                ...
                logger.debug(u"Plex:CS ActivityProcessor :: ratingKey %s not logged. Does not meet logging criteria. "
            ...
            if plexcs.CONFIG.LOGGING_IGNORE_INTERVAL and not is_import:
                if (session['media_type'] == 'movie' or session['media_type'] == 'episode') and \
                    ...
                    logging_enabled = False
                    logger.debug(u"Plex:CS ActivityProcessor :: Play duration for ratingKey %s is %s secs which is less than %s "
                                 u"seconds, so we're not logging it." %
                                 (session['rating_key'], str(real_play_time), plexcs.CONFIG.LOGGING_IGNORE_INTERVAL))
            if session['media_type'] == 'track' and not is_import:
                if real_play_time < 15 and session['duration'] >= 30:
                    logging_enabled = False
                    logger.debug(u"Plex:CS ActivityProcessor :: Play duration for ratingKey %s is %s secs, "
            ...
            if not user_details['keep_history'] and not is_import:
                logging_enabled = False
                logger.debug(u"Plex:CS ActivityProcessor :: History logging for user '%s' is disabled." % session['user'])
            ...
            if logging_enabled:
                ...
                if not is_import:
                    logger.debug(u"Plex:CS ActivityProcessor :: Fetching metadata for item ratingKey %s" % session['rating_key'])
    ...
    def find_session_ip(self, rating_key=None, machine_id=None):
        ...
        logger.debug(u"Plex:CS ActivityProcessor :: Requesting log lines...")
            ...
            if machine_id_line in line and (rating_key_line in line or rating_key_line_2 in line):
                ...
                if ipv4:
                    ...
                    if ipv4[0] != '127.0.0.1':
                        ...
                        #    logger.debug(u"Plex:CS ActivityProcessor :: Matched IP address (%s) for stream ratingKey %s "
        ...
        logger.debug(u"Plex:CS ActivityProcessor :: Unable to find IP address on first pass. "
        ...
        logger.debug(u"Plex:CS ActivityProcessor :: Unable to find IP address on fallback search. Not logging IP address.")
